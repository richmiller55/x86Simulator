...
void CodeGenerator::process_line(const std::string& line_raw) {
  ...
  if (mnemonic == "nop") {
    ...
    } else if (dest == "edx") { // Assuming immediate
      machine_code_.push_back(0x83);
      machine_code_.push_back(0xFA); // ModR/M for CMP EDX, imm8
      current_address_ += 2;
      try {
	uint8_t value = std::stoul(src, nullptr, 0);
	machine_code_.push_back(value);
    ...
    if (it != symbol_table_.end()) {
      address_t target_address = it->second;
      int32_t offset = target_address - (current_address_ + 6); // 6 bytes for jne rel32
      machine_code_.push_back(0x0F);
      machine_code_.push_back(0x85);
    ...
    if (it != symbol_table_.end()) {
      address_t target_address = it->second;
      int32_t offset = target_address - (current_address_ + 6); // 6 bytes for je rel32
      machine_code_.push_back(0x0F);
      machine_code_.push_back(0x84);
      machine_code_.insert(machine_code_.end(), { (uint8_t)offset, (uint8_t)(offset >> 8), (uint8_t)(offset >> 16), (uint8_t)(offset >> 24) });
      current_address_ += 6;
    } else {
      // Sizing pass for a forward reference. Assume near jump size.
    ...
    if (it != symbol_table_.end()) {
      address_t target_address = it->second;
      int32_t offset = target_address - (current_address_ + 6); // 6 bytes for jb rel32
      machine_code_.push_back(0x0F);
      machine_code_.push_back(0x82);
      machine_code_.insert(machine_code_.end(), { (uint8_t)offset, (uint8_t)(offset >> 8), (uint8_t)(offset >> 16), (uint8_t)(offset >> 24) });
      current_address_ += 6;
    } else {
      // Sizing pass for a forward reference. Assume near jump size.
    ...
    if (it != symbol_table_.end()) {
      address_t target_address = it->second;
      int32_t offset = target_address - (current_address_ + 6); // 6 bytes for jae rel32
      machine_code_.push_back(0x0F);
      machine_code_.push_back(0x83);
      machine_code_.insert(machine_code_.end(), { (uint8_t)offset, (uint8_t)(offset >> 8), (uint8_t)(offset >> 16), (uint8_t)(offset >> 24) });
      current_address_ += 6;
    ...
    if (it != symbol_table_.end()) {
      address_t target_address = it->second;
      int32_t offset = target_address - (current_address_ + 6); // 6 bytes for jbe rel32
      machine_code_.push_back(0x0F);
      machine_code_.push_back(0x86);
      machine_code_.insert(machine_code_.end(), { (uint8_t)offset, (uint8_t)(offset >> 8), (uint8_t)(offset >> 16), (uint8_t)(offset >> 24) });
      current_address_ += 6;
    } else {
      // Sizing pass for a forward reference. Assume near jump size.
    ...
    if (it != symbol_table_.end()) {
      address_t target_address = it->second;
      int32_t offset = target_address - (current_address_ + 6); // 6 bytes for js rel32
      machine_code_.push_back(0x0F);
      machine_code_.push_back(0x88);
      machine_code_.insert(machine_code_.end(), { (uint8_t)offset, (uint8_t)(offset >> 8), (uint8_t)(offset >> 16), (uint8_t)(offset >> 24) });
      current_address_ += 6;
    } else {
      // Sizing pass for a forward reference. Assume near jump size.
    ...
    if (it != symbol_table_.end()) {
      address_t target_address = it->second;
      int32_t offset = target_address - (current_address_ + 6); // 6 bytes for jns rel32
      machine_code_.push_back(0x0F);
      machine_code_.push_back(0x89);
      machine_code_.insert(machine_code_.end(), { (uint8_t)offset, (uint8_t)(offset >> 8), (uint8_t)(offset >> 16), (uint8_t)(offset >> 24) });
      current_address_ += 6;
    ...
    if (it != symbol_table_.end()) {
      address_t target_address = it->second;
      int32_t offset = target_address - (current_address_ + 6); // 6 bytes for jo rel32
      machine_code_.push_back(0x0F);
    ...
    if (it != symbol_table_.end()) {
      address_t target_address = it->second;
      int32_t offset = target_address - (current_address_ + 6); // 6 bytes for jno rel32
      machine_code_.push_back(0x0F);
      machine_code_.push_back(0x81);
      machine_code_.insert(machine_code_.end(), { (uint8_t)offset, (uint8_t)(offset >> 8), (uint8_t)(offset >> 16), (uint8_t)(offset >> 24) });
      current_address_ += 6;
    } else {
      // Sizing pass for a forward reference. Assume near jump size.
    ...
    if (it != symbol_table_.end()) {
      address_t target_address = it->second;
      int32_t offset = target_address - (current_address_ + 6); // 6 bytes for jl rel32
      machine_code_.push_back(0x0F);
      machine_code_.push_back(0x8C);
      machine_code_.insert(machine_code_.end(), { (uint8_t)offset, (uint8_t)(offset >> 8), (uint8_t)(offset >> 16), (uint8_t)(offset >> 24) });
      current_address_ += 6;
    } else {
      // Sizing pass for a forward reference. Assume near jump size.
    ...
    if (it != symbol_table_.end()) {
      address_t target_address = it->second;
      int32_t offset = target_address - (current_address_ + 6); // 6 bytes for jge rel32
      machine_code_.push_back(0x0F);
      machine_code_.push_back(0x8D);
      machine_code_.insert(machine_code_.end(), { (uint8_t)offset, (uint8_t)(offset >> 8),
	...
	current_address_ += 6;
      ...
      if (it != symbol_table_.end()) {
	address_t target_address = it->second;
	int32_t offset = target_address - (current_address_ + 6); // 6 bytes for jg rel32
	machine_code_.push_back(0x0F);
	machine_code_.push_back(0x8F);
	machine_code_.insert(machine_code_.end(), { (uint8_t)offset, (uint8_t)(offset >> 8), (uint8_t)(offset >> 16), (uint8_t)(offset >> 24) });
	current_address_ += 6;
      } else {
	// Sizing pass for a forward reference. Assume near jump size.
	...
	// VEX byte 2: [R|vvvv|L|pp]
	// R=1 (for ymm0-7), vvvv=1111 (not used), L=1 (256-bit), pp=00
	uint8_t vex_byte2 = (1 << 7) | (0b1111 << 3) | (1 << 2) | 0;
	machine_code_.push_back(vex_byte2);

	// Opcode
	machine_code_.push_back(0x53);
